//
//  AthleteScheduleStreamViewController.swift
//  Athletica
//
//  Created by SilverStar on 7/25/17.
//  Copyright © 2017 ClearAppDevelopment. All rights reserved.
//

import UIKit

class AthleteScheduleStreamViewController: BaseViewController  , UIPickerViewDelegate, UIPickerViewDataSource{
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        self.lblInvites.text = String(self.invitedCoaches.count)
    ...
    @IBAction func valueChangedForPicker(sender: UIDatePicker) {
        ...
        if now.compare(streamTime) == ComparisonResult.orderedDescending {
            sender.date = now            
        }
    ...
    func scheduleStream(){
        ...
        stream.isSaveStream = self.switchSaveStream.isOn
        
        var invitedCoaches = [String:String]()
        for item in self.invitedCoaches {
            invitedCoaches[item.userId] = item.userId
        }
        stream.invitedCoaches = invitedCoaches
    ...
    @IBAction func btnInviteTapped(_ sender: UIButton) {
        ...
        // Return if not purchased
        let timestamp = UserDefaults.standard.double(forKey: "expiryTimestamp")
        if !isPurchasedAthlete(timestamp: timestamp){
            showAlert(title: nil, message: "Please upgrade to Premium", controller: self, okTitle: "OK", cancelTitle: nil, okAction: nil, cancelAction: nil)
            return
        }
        
        let storyboard = UIStoryboard.init(name: "Main", bundle: nil)
        let vc = storyboard.instantiateViewController(withIdentifier: "AthleteInviteCoachesViewController") as! AthleteInviteCoachesViewController
        vc.delegate = self
